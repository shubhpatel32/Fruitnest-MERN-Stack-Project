    ...
    return (
        <header className="fixed top-0 left-0 right-0 w-full h-[6rem] z-50 bg-gradient-to-l from-red-700 to-yellow-400 flex items-center justify-between px-8 py-4 md:px-16 md:py-8">
            ...
            <div className="flex items-center justify-between w-full md:w-auto">
                <nav
                    ref={menuRef}
                    className={`absolute top-[50px] shadow-sm shadow-gray-500 md:shadow-none flex-col md:flex-row md:flex ${isMenuOpen ? 'flex bg-white gap-3' : 'hidden'} md:flex items-center text-[1.5rem] md:text-2xl text-white font-medium w-full left-0 md:items-center md:justify-center md:top-[10px] md:w-auto md:static divide-y p-4`}
                >
            ...
            <div className="flex text-center justify-center items-center">
                ...
                {isLoggedIn ? (
                            ...
                            {isOpen && (
                                ...
                                >
                                    <li>
                                        <Link
                                            to="/profile"
                                            className="block px-4 py-2 text-[1.5rem] md:text-[1.5rem] text-black hover:bg-gray-400 m-2 bg-gray-300 rounded-lg"
                                            onClick={toggleDropdown}
                                        >
                                            My Profile
                                        </Link>
                    ...
                    <NavLink to="/login" id="login-btn" className="text-[1.7rem] ml-4 cursor-pointer text-black font-medium p-2 normal-case border border-solid rounded-lg w-32">
                        Login
                    </NavLink>
                )}
            </div>
